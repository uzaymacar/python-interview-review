# STACKS

# Stacks are useful in certain recursion algorithms
class StackNode(object):
    def __init__(self, val = None):
        self.data = val
        self.next = None
    
    def get_value(self):
        return self.data

class Stack(object): # stack of dinner plates, LIFO!
    def __init__(self, first = None):
        self.top = first
        # self.min_set = set()
        # self.min_element = first.data
        self.capacity = 5
        self.current = 0
        self.stackCount = 1
        self.array = []
        self.array.append(self.top)
        
    def pop(self):
        self.current -= 1
        if self.top == None:
            return None
        item = self.top.data
        self.top = self.top.next
        return item
    
    def push(self, item):
        self.current += 1
            
        # if (item < self.min_element):
            # self.min_element = item
        # self.min_set.add(item)
        new_node = StackNode(item)
        if self.current == self.capacity + 1:
            self.stackCount += 1
            self.array.append(new_node)
            self.capacity *= 2
        else:
            if self.current - self.capacity < 0:
                new_node.next = self.array[0]
                self.array[0] = new_node
            elif self.current - self.capacity > 0: 
                new_node.next = self.array[self.stackCount - 1]
                self.array[self.stackCount - 1] = new_node
                
        
    def peek(self):
        if self.top == None:
            return None
        return self.top.data
    
    def isEmpty(self):
        return self.top == None
    
    def get_min(self):
        return self.min_element
        # print(self.min_set)
        # return list(self.min_set)[0]
               
stack = Stack(StackNode(5))
stack.push(4)
stack.push(3)
stack.push(15)
stack.push(25)